...
y_pr_lin = lin_clf.predict(x_train)
print(f'svm.SVC predict: {np.array(y_train) - np.array(y_pr)}')  # нули - без ошибок; иначе - ошибка
print(f'svm.LinearSVC predict: {np.array(y_train) - np.array(y_pr_lin)}')
...
plt.scatter(v[:, 0], v[:, 1], s=70, edgecolor=None, linewidths=0, marker='s', c='y')
